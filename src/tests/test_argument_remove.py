...
class TestArgumentRemove:
    ...
    def test_remove_step_from_argument_with_justifiers(self):
        ...
        with patch.object(coordinator, 'queue_task') as mock_queue:
            ...
            assert args.argument[0].symbol == "1"
            assert args.argument[1].symbol == "3"

            # Verify justifiers were properly transferred
            # Step 3 should now have justifiers ["1"] (2's justifiers) instead of ["2"]
            assert args.argument[1].justifiers == ["1"]
    ...
    def test_remove_step_from_argument_without_justifiers(self):
        ...
        with patch.object(coordinator, 'queue_task') as mock_queue:
            ...
            # Verify justifiers were not changed (2 had no justifiers)
            assert args.argument[1].justifiers == ["1"]
    ...
    def test_remove_step_with_complex_justifier_transfer(self):
        ...
        with patch.object(coordinator, 'queue_task') as mock_queue:
            ...
            assert args.argument[0].symbol == "1"
            assert args.argument[1].symbol == "2"
            assert args.argument[2].symbol == "4"

            # Verify justifiers were properly transferred
            # Step 4 should now have justifiers ["1", "2"] (3's justifiers) instead of ["3"]
            assert set(args.argument[2].justifiers) == {"1", "2"}
    ...
    def test_remove_step_preserves_other_arguments(self):
        ...
        with patch.object(coordinator, 'queue_task'):
            ...
            assert args.argument[0].symbol == "1"
            assert args.argument[0].proposition == "Step 1"

            # Verify the removed step is no longer present
            step_2_symbols = [step.symbol for step in args.argument if step.symbol == "2"]
            assert len(step_2_symbols) == 0
